...
def redist_via_clamp(current_label, poss_classes, class2idx, redist_factor=0.1, smooth_existing=False):
    ...
    n_current_non_zero = np.sum(current_label > 0.0) # see how many labels to spread over (1 or 2 in current set-up)
    ...
    if len(poss_classes) - n_current_non_zero != 0:
        ...
        for poss_label in poss_classes: 
            if current_label[class2idx[poss_label]] == 0: # only place if already zero prob (o.w. added)
        ...
        if smooth_existing: 
            ...
            for class_idx, current_label_val in enumerate(current_label):
                if current_label_val != 0:
                    adjusted_prob_labels[class_idx] = current_label_val * (1-redist_factor)
        else: adjusted_prob_labels = current_label # no change
        adjusted_label = adjusted_prob_labels + redist_label # add the equiv of a redist masker
...
def construct_elicited_soft_label(elicited_data, class2idx, idx2class, include_top_2 = True, redist="clamp", redist_factor=0.1):
    ...
    if allocated_mass < 1: 
        ...
        n_leftover_spread = len(poss_classes) -  np.sum(subj_label > 0.0) # see how many labels to spread over (1 or 2 in current set-up)
    ...
    if redist == "clamp": 
        ...
        subj_label = redist_via_clamp(subj_label, poss_classes, class2idx, redist_factor=redist_factor,smooth_existing=smooth_existing)
    elif redist == "uniform": 
        # spread mass uniformly over all other classes
        subj_label = apply_uniform_redist(subj_label, redist_factor=redist_factor) 
